...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--jailbreak-dataset",
        default="active_jailbreaks",
        help="Jailbreak dataset to use.",
        choices=["active_jailbreaks"]
    )
    parser.add_argument(
        "--output-dir",
    ...
    with open(f"data/{args.jailbreak_dataset}.pkl", "rb") as f:
        jailbreak_dataset = pickle.load(f)

    for input_sequence in jailbreak_dataset["input_sequence"]:
        jailbreak_dataset["templated_prompts"].append(
            target_lm.model.tokenizer.apply_chat_template([{"role": "user", "content": input_sequence}], 
                                                          tokenize=False, 
                                                          add_generation_prompt=True))
    
    flattened_dict_dataset = []
    for idx in range(len(jailbreak_dataset["templated_prompts"])):
        datum = {}
        for key in jailbreak_dataset.keys():
            datum[key] = jailbreak_dataset[key][idx]
        flattened_dict_dataset.append(datum)

    jailbreak_dataloader = DataLoader(flattened_dict_dataset, batch_size=batch_size, shuffle=False, collate_fn=custom_collate_fn)
    prompt_key = "templated_prompts"

    batch_responses = []
    for batch in tqdm(jailbreak_dataloader):
        _batch = copy.deepcopy(batch)
        responses = target_lm.get_response(
            batch["templated_prompts"],
        ...
        print(sum(_batch['jailbroken_rule_judge']))

    # create nice data structure from batch responses
    prompt_responses = {}
    for batch in batch_responses:
        for i in range(len(batch["response"])):
            ...
            prompt_responses[batch["source_model"][i] + "--" + batch["instruction"][i]] = example_dict

    # save those responses
    with open(f'{args.output_dir}/{args.target_model}-active_jailbreak_responses.pkl', 'wb') as f:
    ...
    #with open(f'{args.output_dir}/{args.target_model}-short_responses.pkl', 'wb') as f:
    #    pickle.dump(batch_responses, f)

    #with open(f'{args.output_dir}/{args.target_model}-short_responses.pkl', 'rb') as f:
    ...
    #with open(f'{args.output_dir}/{args.target_model}-long_responses.pkl', 'wb') as f:
    #    pickle.dump(prompt_responses, f)

    #with open(f'{args.output_dir}/{args.target_model}-long_responses.pkl', 'rb') as f:
    ...
    #batch_responses = []
    #response_dataloader = DataLoader(list(augmented_prompt_responses.values()), batch_size=batch_size, shuffle=False, collate_fn=custom_collate_fn)
