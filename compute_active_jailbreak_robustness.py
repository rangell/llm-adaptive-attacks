...
if __name__ == '__main__':
    ...
    jailbreak_dataset["jailbreak_prompt_text"] = target_lm.get_full_prompts(jailbreak_dataset["input_sequence"])
    
    flattened_dict_dataset = []
    for idx in range(len(jailbreak_dataset["jailbreak_prompt_text"])):
        datum = {}
        for key in jailbreak_dataset.keys():
            datum[key] = jailbreak_dataset[key][idx]
        # TODO: add original prompt text
        datum["original_prompt_text"] = jailbreak_dataset["instruction"][idx].replace("_", " ").replace("+", "'").capitalize()
        flattened_dict_dataset.append(datum)

    jailbreak_dataset = flattened_dict_dataset
